...
function select(){
  ...
  return s;
        ...
        const box = ensureBox(core, "p030", "Video actions");
        if(!box) return;
        const row = box.querySelector(".row");
        const v = core.video;
        const b = btn("Open video");
        b.addEventListener("click", ()=>{
          const url = (v?.src||"").trim();
          if(!url) return core.toast("Brak video");
          window.open(url, "_blank", "noopener");
        });
        row.appendChild(b);
        core.toast("Loaded p030");
      }catch(e){ core.log({plugin:"p030", error:String(e)}); }
    }
  };
})();
