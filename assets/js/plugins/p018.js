...
function select(){
  ...
  return s;
        ...
        const box = ensureBox(core, "p018", "Console");
        if(!box) return;
        const row = box.querySelector(".row");
        const log = core.log;
        const b = btn("Toggle log");
        b.addEventListener("click", ()=>{
          if(!log) return core.toast("Brak #log");
          log.style.display = (log.style.display === "none") ? "block" : "none";
          core.toast("OK");
        });
        row.appendChild(b);
        core.toast("Loaded p018");
      }catch(e){ core.log({plugin:"p018", error:String(e)}); }
    }
  };
})();
