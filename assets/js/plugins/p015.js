...
function select(){
  ...
  return s;
        ...
        const box = ensureBox(core, "p015", "Backend host");
        if(!box) return;
        ...
        function upd(){
          ...
          try{ pill.textContent = new URL(v).host; }catch(_){ pill.textContent = v.replace(/^https?:\/\//,""); }
        }
        backend?.addEventListener("input", upd);
        backend?.addEventListener("change", upd);
        upd();
        core.toast("Loaded p015");
      }catch(e){ core.log({plugin:"p015", error:String(e)}); }
    }
  };
})();
