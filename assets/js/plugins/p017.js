...
function select(){
  ...
  return s;
        ...
        const box = ensureBox(core, "p017", "Random suffix");
        if(!box) return;
        const row = box.querySelector(".row");
        const prompt = core.prompt;
        const list = ["cinematic lighting","seamless loop","macro close-up","slow zoom in","high detail","clean background","subtle motion","handheld"];
        const b = btn("Append random");
        b.addEventListener("click", ()=>{
          if(!prompt) return;
          const add = list[Math.floor(Math.random()*list.length)];
          prompt.value = ((prompt.value||"").trim() + " " + add).trim();
          prompt.dispatchEvent(new Event("input")); prompt.dispatchEvent(new Event("change"));
          core.toast("Dodano: " + add);
        });
        row.appendChild(b);
        core.toast("Loaded p017");
      }catch(e){ core.log({plugin:"p017", error:String(e)}); }
    }
  };
})();
