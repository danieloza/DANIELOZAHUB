...
function select(){
  ...
  return s;
        ...
        const box = ensureBox(core, "p033", "Prompt lint");
        if(!box) return;
        const row = box.querySelector(".row");
        const prompt = core.prompt;
        const b = btn("Check duplicates");
        b.addEventListener("click", ()=>{
          const t = (prompt?.value||"").toLowerCase().trim();
          if(!t) return core.toast("Prompt pusty");
          const w = t.split(/\s+/);
          const dup = [];
          for(let i=1;i<w.length;i++) if(w[i]===w[i-1]) dup.push(w[i]);
          if(dup.length){
            core.toast("Powtórzenia: " + dup.slice(0,6).join(", "));
            core.log({duplicates: dup});
          }else{
            core.toast("OK ✅");
          }
        });
        row.appendChild(b);
        core.toast("Loaded p033");
      }catch(e){ core.log({plugin:"p033", error:String(e)}); }
    }
  };
})();
