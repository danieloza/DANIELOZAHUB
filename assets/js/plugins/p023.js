...
function select(){
  ...
  return s;
        ...
        const box = ensureBox(core, "p023", "Auto health");
        if(!box) return;
        ...
        async function ping(){
          ...
          if(!base){ pill.textContent="(no backend)"; return; }
          const t0 = performance.now();
          try{
            const r = await fetch(${base}/api/health, {cache:"no-store"});
            const ms = Math.round(performance.now()-t0);
            pill.textContent = r.ok ? ok (ms) : err ();
          }catch(_){
        ...
        core.toast("Loaded p023");
      }catch(e){ core.log({plugin:"p023", error:String(e)}); }
    }
  };
})();
