...
function select(){
  ...
  return s;
        ...
        const box = ensureBox(core, "p012", "Image URL tools");
        if(!box) return;
        const row = box.querySelector(".row");
        const url = core.image_url;
        const b = btn("Copy image_url");
        b.addEventListener("click", async ()=>{
          const t = (url?.value||"").trim();
          if(!t) return core.toast("Brak image_url");
          try{ await navigator.clipboard.writeText(t); core.toast("Skopiowano URL ✅"); }catch(_){ core.toast("Clipboard blocked"); }
        });
        row.appendChild(b);
        core.toast("Loaded p012");
      }catch(e){ core.log({plugin:"p012", error:String(e)}); }
    }
  };
})();
